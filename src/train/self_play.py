...
class SelfPlayTrainerArgs:
    ...
    max_training_examples: int = 1024 * 10
    board_size: int = 19
    player_count: int = 2
    debug: bool = False
...
class SelfPlayTrainer:
    ...
    def __play_game(self):
        ...
        while True:
            ...
            is_terminal, winner = self.game.check_game_end(board, next_player)
            if is_terminal:
    ...
    def __build_training_examples(self):
        training_examples = []
    ...
    def train(self):
        logger.info("Entered self play training loop")
        for iteration in range(self.args.training_iterations):
            iter_start_time = time.time()
            logger.info(f"=== Iteration {iteration + 1}/{self.args.training_iterations} ===")

            logger.info("Generating training examples from self-play games...")
            sp_start_time = time.time()
            training_examples = self.__build_training_examples()
    ...
    def __train_model(self, training_examples: list[tuple[np.ndarray, np.ndarray, float]]) -> ModelTrainingStats:
        ...
        for batch_idx, batch in enumerate(dataloader, 1):
            ...
            p_loss = self.policy_loss(pred_policies_logits, target_policies.type(torch.float32))
